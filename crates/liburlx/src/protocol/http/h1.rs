    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    let server_wants_close =
        ph.headers.get("connection").is_some_and(|v| v.eq_ignore_ascii_case("close"));
    // When --fail skips the body, unread data remains on the socket. Don't reuse
    // the connection if the response had body framing (Content-Length or chunked),
    // because the next request would read stale body bytes as headers (curl compat: test 1328).
    let has_body_framing = ph.headers.contains_key("content-length")
        || ph.headers.get("transfer-encoding").is_some_and(|te| te_contains_chunked(te));
    ...
    S: AsyncRead + Unpin,
{
    ...
    // Determine the outermost encoding for incremental checking and stall detection.
    //
    // For gzip/deflate: `is_encoding_corrupt()` checks incrementally after each read.
    // For brotli/zstd: decompressors can't distinguish truncated from corrupt on
    //   partial streams, so only stall-based detection works.
    //
    // All encodings use a stall timeout as a safety net: if the server stops sending
    // data (Content-Length > actual bytes), we try decompression after a timeout
    // rather than blocking forever (curl compat: test 223 — broken deflate with
    // Content-Length mismatch, test 315 — broken brotli).
    while body.len() < content_length {
        let remaining = content_length - body.len();
        ...
        // If we already have data and the next read stalls, try decompression
        // to detect corrupt encoding without blocking forever. This handles
        // Content-Length > actual data for all encodings:
        // - deflate/gzip: `is_encoding_corrupt()` catches most broken streams
        //   immediately, but stall timeout is the safety net for valid-but-truncated
        //   streams where the server stops sending
...
mod tests {
    ...
    fn is_encoding_corrupt_detects_full_test223_deflate_payload() {
        ...
        // bytes removed from the beginning, then cut short. Content-Length is
        // 1305 but only these 412 bytes arrive before the server stalls.
        #[rustfmt::skip]
    ...
    async fn read_exact_body_with_encoding_check_detects_corrupt_deflate() {
        ...
        // Simulate test 223: server sends full 412 bytes of broken deflate
        // data (three bytes removed from the beginning, cut short), Content-
        // Length is 1305, and the TCP connection stays open. The reader must
        // detect corrupt encoding and bail instead of blocking forever.
        #[rustfmt::skip]
        let broken_deflate: &[u8] = &[
            0x58, 0xdb, 0x6e, 0xe3, 0x36, 0x10, 0x7d, 0x37, 0x90, 0x7f, 0x60, 0xfd,
            0xd4, 0x02, 0xb6, 0x6e, 0xb6, 0x13, 0x39, 0x70, 0xb4, 0x28, 0x72, 0xd9,
    ...
    async fn read_exact_body_encoding_check_with_data_in_prefix() {
        // When broken deflate data arrives entirely in the body prefix (already
        // buffered from header read), the encoding check must detect corruption
        // immediately without entering the read loop.
        #[rustfmt::skip]
        ...
        // Empty stream — all data is in the prefix
    ...
    async fn read_exact_body_with_encoding_check_detects_corrupt_deflate_full_data() {
        ...
        // Full test 223 broken deflate data: valid deflate with first 3 bytes removed
        // (zlib header 0x78 0x9c and first deflate byte 0xdc removed), then truncated.
        // Content-Length: 1305 but only 412 bytes of actual data.
        let broken_deflate: &[u8] = &[
        ...
        let (mut writer, mut reader) = tokio::io::duplex(4096);
        writer.write_all(broken_deflate).await.unwrap();
        // Keep writer alive — simulates server holding keep-alive connection open
    ...
    async fn read_exact_body_with_encoding_check_detects_corrupt_in_prefix() {
        // When broken data arrives in the body prefix (common when headers + body
        // are read in one TCP segment), corruption should be detected immediately
        // without any stream reads.
        let broken_deflate: Vec<u8> = vec![
            0x58, 0xdb, 0x6e, 0xe3, 0x36, 0x10, 0x7d, 0x37, 0x90, 0x7f, 0x60, 0xfd, 0xd4, 0x02,
            0xb6, 0x6e, 0xb6, 0x13, 0x39, 0x70, 0xb4, 0x28, 0x72, 0xd9, 0x04, 0xcd, 0x36, 0xc1,
        ];
        ...
        // Use a stream that would block forever if read
    ...
    fn is_encoding_corrupt_detects_bad_zlib_header() {
        // Data with invalid zlib header (CMF*256 + FLG) % 31 != 0
        // This is the exact start of test 223 data, which fails both raw deflate
        // and zlib validation.
    ...
    fn is_encoding_corrupt_handles_multi_layer_encoding() {
        // Multi-layer encoding: outermost (rightmost) is checked
        let bad_data = b"\x58\xdb\x6e\xe3\x36\x10";
        // "identity, deflate" — outermost is deflate, should detect corruption
        assert!(is_encoding_corrupt(bad_data, "identity, deflate"));
    }

    #[test]
    fn is_encoding_corrupt_empty_data_deflate() {
        // Empty data with deflate encoding — not flagged as corrupt because
        // DeflateDecoder treats empty input as a valid (empty) stream.
        assert!(!is_encoding_corrupt(b"", "deflate"));
    }

    #[test]
    fn is_encoding_corrupt_single_byte_deflate() {
        // A single byte may look like a valid partial deflate stream start,
        // so is_encoding_corrupt may not flag it. Corruption is only detected
        // when there's enough data to conclusively determine the stream is invalid.
        // 0xFF with LSB-first: BFINAL=1, BTYPE=11 (reserved) — flate2 may accept or reject
