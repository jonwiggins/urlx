...
impl CookieJar {
    ...
    pub fn save_to_writer<W: Write>(&self, writer: &mut W) -> std::io::Result<()> {
        ...
        writeln!(writer, "# This file was generated by urlx! Edit at your own risk.")?;
        writeln!(writer)?;

        for cookie in &self.cookies {
            // Domain: Netscape format uses leading dot for domain cookies
            let domain_str = if cookie.http_only {
                format!("#HttpOnly_.{}", cookie.domain)
            } else {
                format!(".{}", cookie.domain)
            };
            // Include subdomains: TRUE if domain has leading dot (always TRUE here)
            let flag = "TRUE";
...
mod tests {
    ...
    fn save_format_fields() {
        ...
        assert_eq!(fields[0], ".example.com"); // domain with dot prefix
        assert_eq!(fields[1], "TRUE"); // include subdomains
