...
fn store_multiple_cookies() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["a=1", "b=2", "c=3"], "example.com", "/");
    assert_eq!(jar.len(), 3);
}
...
fn store_cookie_with_domain() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["token=xyz; Domain=example.com"], "www.example.com", "/");
    assert_eq!(jar.len(), 1);
...
fn store_cookie_with_path() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["api_key=123; Path=/api"], "example.com", "/");
    assert_eq!(jar.len(), 1);
...
fn cookie_header_returns_none_for_non_matching_domain() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["a=1"], "example.com", "/");
...
fn cookie_header_joins_multiple_cookies() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["a=1", "b=2"], "example.com", "/");
...
fn cookie_header_secure_filtering() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["secure_token=abc; Secure"], "example.com", "/");
...
fn store_replaces_cookie_with_same_name() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["key=old"], "example.com", "/");
    jar.store_cookies(&["key=new"], "example.com", "/");
    assert_eq!(jar.len(), 1);
...
fn remove_expired_clears_max_age_zero() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["temp=1"], "example.com", "/");
    assert_eq!(jar.len(), 1);

    // Set max-age=0 to expire it
    jar.store_cookies(&["temp=1; Max-Age=0"], "example.com", "/");
    jar.remove_expired();
    assert!(jar.is_empty());
}
...
fn remove_expired_keeps_valid_cookies() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["valid=1; Max-Age=3600"], "example.com", "/");
    jar.store_cookies(&["session=2"], "example.com", "/");
    assert_eq!(jar.len(), 2);
...
fn clone_is_independent() {
    ...
    cloned.store_cookies(&["b=2"], "example.com", "/");
...
fn cookie_with_equals_in_value() {
    let mut jar = CookieJar::new();
    jar.store_cookies(&["data=base64==abc"], "example.com", "/");
    assert_eq!(jar.len(), 1);
}
...
fn store_from_headers_with_multiple_set_cookies() {
    ...
    jar.store_from_headers(&headers, "example.com", "/");
    assert_eq!(jar.len(), 2);
}
